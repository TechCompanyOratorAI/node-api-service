...
const QUEUE_URLS = {
  ...
  analysis: process.env.AWS_SQS_SEMANTIC_QUEUE_URL,
...
class QueueService {
  ...
  async _sendMessage(queueUrl, queueType, data) {
    if (!queueUrl) {
      throw new Error(
        `Queue URL not configured for ${queueType}. Please set AWS_SQS_${queueType.toUpperCase()}_QUEUE_URL`,
      );
    }
    ...
    try {
      ...
      console.error(
        `[QueueService] Failed to send message to ${queueType} queue:`,
        error,
      );
      throw new Error(
        `Failed to send message to ${queueType} queue: ${error.message}`,
      );
    }
  }
  ...
  async receiveMessages(queueType, maxMessages = 1, waitTimeSeconds = 20) {
    ...
    try {
      ...
      console.error(
        `[QueueService] Failed to receive messages from ${queueType} queue:`,
        error,
      );
      throw error;
    }
