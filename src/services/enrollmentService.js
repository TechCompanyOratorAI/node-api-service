...
const { Enrollment, EnrollKey, Class, Course, User, ClassInstructor, Topic, TopicEnrollment } = db;
...
class EnrollmentService {
    ...
    async joinClass(keyValue, studentId) {
        ...
        try {
            ...
            return { success: false, message: 'Không thể đăng ký lớp học', error: error.message };
        }
    }
    ...
    async leaveClass(classId, studentId) {
        try {
            ...
            return { success: false, message: 'Không thể rời lớp học', error: error.message };
        }
    }
    ...
    async getClassStudents(classId, userId, userRole) {
        try {
            ...
            if (userRole !== 'Admin') {
                ...
                if (!isInstructor) {
                    return { success: false, message: 'Bạn không có quyền xem danh sách sinh viên' };
                }
            }
            ...
            return { success: false, message: 'Không thể lấy danh sách sinh viên', error: error.message };
        }
    }
    ...
    async enrollTopic(topicId, studentId) {
        try {
            ...
            if (!classEnrollment) {
                return { success: false, message: 'You must enroll in a class before enrolling in a topic' };
            }
    ...
    async listMyTopics(studentId) {
        try {
            ...
            const courses = await Course.findAll({
                ...
                include: [{ model: User, as: 'instructors', through: { attributes: [] }, attributes: ['userId', 'username', 'firstName', 'lastName'] }]
            });
